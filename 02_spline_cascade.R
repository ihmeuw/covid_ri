...
# Author: NAME
# Date: 7/25/2020
# Purpose: Run splines to calculate vaccine interruption
#          
...
mobility_version <- "VERSION_NUMBER"  # Change this to reflect the correct version of mobility from Steve and the covid team 

data_dir <- "VERSION_NUMBER"
...
work_dir <- paste0(j_root, "FILEPATH", "cumulative_", Sys.Date(), "_", model_name)  # Outputs will be stored here
dir.create(work_dir)
...
pdf(paste0(work_dir, "/spline_fit_plots.pdf"), width = 11, height = 8.5)
  ...
  # # SENSITIVITY drop last 20% of data
  # dt[, max_month := max(month), by = c("ihme_loc_id", "dtp", "mcv", "source_id")]
  
  # write.csv(dt[(max_month %in% 10:12 & month > (max_month-2)) | (max_month %in% 5:9 & month > (max_month-1))],
  #           file.path(work_dir, "dropped_data.csv"), row.names = F)
  
  # dt <- dt[!(max_month %in% 10:12 & month>max_month-2)]
  # dt <- dt[!(max_month %in% 5:9 & month>max_month-1)]
  
  # Read in covariates and merge on -------------------------------------------
  
  # mobility
  dt <- merge(dt, mob, by = c("location_id", "date"), all.x = T)
  
  # Change all subnat locations to parent
  for(loc_id in unique(dt$location_id)){
    if(!loc_id %in% hierarchy[level ==3, location_id]){
      
      iso3 <- strsplit(dt[location_id == loc_id, unique(ihme_loc_id)], "_")[[1]][1]
      
      dt[location_id == loc_id, location_id:= hierarchy[ihme_loc_id == iso3, location_id]]
    }
  }
  
  dt <- merge(dt, hierarchy[,.(location_id, super_region_id, super_region_name)], by = "location_id", all.x = T)
  ...
  #the SE values are extreme, so set caps
  ...
  for(r_name in unique(preds2[!is.na(super_region_name), super_region_name])){
  ...
  if(uncertainty){
    ...
    final_pred_sum_dtp <- fread(paste0(work_dir, "/vaccine_dtp/summary_results.csv"))
    final_pred_sum_dtp[, antigen := "dtp"]
    
    final_pred_sum_mcv <- fread(paste0(work_dir, "/vaccine_mcv/summary_results.csv"))
    final_pred_sum_mcv[, antigen := "mcv"]
    
    
    final_pred_sum <- rbind(final_pred_sum_dtp, final_pred_sum_mcv)
    
    
    
  }
  
  pdf(paste0(work_dir, "/spline_fit_plots_countries.pdf"), width = 10, height = 11)
  
  for(loc_id in unique(c(dt$location_id, countries))){
    
    loc_name <- hierarchy[location_id == loc_id, location_name]
    r_id <- hierarchy[location_id == loc_id, super_region_id]
    r_name <- hierarchy[location_id == loc_id, super_region_name]
    
    gg1 <- ggplot(preds1[location_id == loc_id], aes(x = cum_mob, y = pred))+
      geom_hline(aes(yintercept = 1), linetype = "dotted")+
      geom_line(data = preds1[grepl("r", cascade_prediction_id) & 
                                super_region_name == r_name, .(cum_mob, pred, antigen)],
                aes(linetype = "Region", color = antigen), show.legend = F)+
      geom_line(data = unique(global_pred[, .(cum_mob, pred)]), aes(linetype = "Global"), show.legend = F)+
      labs(title = "Stage 1: Vaccine Disruption v. Mobility",
           x = "Cumulative Mobility Disruption",
           y = "Cumulative Vacccine Disruption")+
      scale_linetype_manual(values = c("Country" = "solid", "Region" = "dashed", "Global" = "dotted"))+
      theme_bw()
    
    if(loc_id %in% dt$location_id){
      gg1 <- gg1 + geom_point(data = dt[location_id == loc_id], aes(y = ratio, size = weight, color = antigen), 
                              alpha = 0.5, show.legend = F)+
        geom_line(aes(color = antigen, linetype = "Country"), show.legend = F)
    }else{
      gg1 <- gg1 + geom_point(data = dt[super_region_name == r_name], aes(y = ratio, size = weight, color = antigen), 
                              alpha = 0.2, show.legend = F)
    }
    
    gg2 <- ggplot(preds2[location_id == loc_id], aes(x = month))+
      geom_hline(aes(yintercept = 0), linetype = "dotted")+
      geom_line(data = preds2[grepl("r", cascade_prediction_id) & super_region_name == r_name, .(month, pred, antigen)],
                aes(y = pred, color = antigen, linetype = "Region"))+
      scale_size(guide = FALSE)+
      geom_line(data = unique(global_pred2[, .(month, pred)]), aes(y = pred, linetype = "Global"))+
      labs(title = "Stage 2: Residual v Time",
           x = "Month",
           y = "Residual (logit space)",
           color = "", linetype = "")+
      scale_linetype_manual(values = c("Country" = "solid", "Region" = "dashed", "Global" = "dotted"))+
    ...
    if(loc_id %in% resid_dt$location_id){
      gg2 <- gg2 + geom_point(data = resid_dt[location_id == loc_id], aes(y = logit_resid, color = antigen, size = logit_ratio_se^(-2)), alpha = 0.5)
    }else{
      gg2 <- gg2 + geom_point(data = resid_dt[super_region_name == r_name],
                              aes(y = logit_resid, size = logit_ratio_se^(-2), color = antigen), alpha = 0.2)
    }
    
    gg3 <- ggplot(mob_nat[location_id == loc_id & date>= as.Date("2020-03-01") & date %in% last_days], 
                  aes(x = as.integer(date - as.Date("2020-03-01")), y = (1-cum_mob)))+
      geom_hline(aes(yintercept = 1), linetype = "dotted")+
      geom_line()+
      scale_x_continuous(breaks = c(30, 91, 152, 213, 274), labels = c("Mar", "May", "Jul", "Sept", "Nov"))+
      labs(title = "Cumulative Mobility v. Time",
           x = "Time",
           y = "Cumulative Mobility Disruption" )+
      theme_bw()
    
    gg4 <- ggplot(pred[location_id == loc_id], aes(x = month))+
      geom_hline(aes(yintercept = 1), linetype = "dotted")+
      geom_line(aes(y = pred.x, linetype = "Stage 1", color = antigen))+
      labs(title = "Cumulative Vaccine Disruption v. Time",
           y = "Cumulative Vaccine Disruption Ratio",
           x = "Month", 
           color = "", shape = "", linetype = "")+
      scale_x_continuous(breaks = c(3, 5, 7, 9, 11), labels = c("Mar", "May", "Jul", "Sept", "Nov"))+
      scale_linetype_manual(values = c("Stage 1" = "dashed", "Stage 2" = "solid"))+
      theme_bw()
    
    if(loc_id %in% dt$location_id){
      gg4 <- gg4 + geom_point(data = dt[location_id == loc_id], aes(y = ratio, shape = source, color = antigen), alpha = 0.1)+
        geom_errorbar(data = dt[location_id == loc_id], aes(ymin = lower, ymax = upper, color = antigen), alpha = 0.1)
    }
    
    if(loc_id %in% resid_dt$location_id){
      gg4 <- gg4 + geom_point(data = resid_dt[location_id == loc_id], aes(y = ratio, shape = source, color = antigen))+
        geom_errorbar(data = resid_dt[location_id == loc_id], aes(ymin = lower, ymax = upper, color = antigen))
    }
    
    gg5 <- ggplot(mob_nat[location_id == loc_id & date>= as.Date("2020-03-01") & date %in% last_days], 
                  aes(x = as.integer(date - as.Date("2020-03-01")), y = (1-mob_avg_month)))+
      geom_hline(aes(yintercept = 1), linetype = "dotted")+
      geom_line()+
      scale_x_continuous(breaks = c(30, 91, 152, 213, 274), labels = c("Mar", "May", "Jul", "Sept", "Nov"))+
      labs(title = "Monthly Mobility v. Time",
           x = "Time",
           y = "Mobility Disruption" )+
      theme_bw()
    
    gg6 <- ggplot(pred[location_id == loc_id], aes(x = month))+
      geom_hline(aes(yintercept = 1), linetype = "dotted")+
      geom_line(aes(y = month_ratio.x, color = antigen, linetype = "Stage 1"))+
      labs(title = "Monthly Vaccine Disruption v. Time",
           y = "Vaccine Disruption Ratio",
           x = "Month", 
           color = "", shape = "", linetype = "")+
      scale_x_continuous(breaks = c(3, 5, 7, 9, 11), labels = c("Mar", "May", "Jul", "Sept", "Nov"))+
      scale_linetype_manual(values = c("Stage 1" = "dashed", "Stage 2" = "solid"))+
    ...
    if(loc_id %in% dt$location_id){
      gg6 <- gg6 + geom_point(data = dt[location_id == loc_id], aes(y = inst_ratio, shape = source, color = antigen), alpha = 0.1)+
        geom_errorbar(data = dt[location_id == loc_id], aes(ymin = lower_inst, ymax = upper_inst, color = antigen), alpha = 0.1)
    }
    
    if(loc_id %in% resid_dt$location_id){
      gg6 <- gg6 + geom_point(data = resid_dt[location_id == loc_id], aes(y = inst_ratio, shape = source, color = antigen))+
        geom_errorbar(data = resid_dt[location_id == loc_id], aes(ymin = lower_inst, ymax = upper_inst, color = antigen))
    }
    
    if(uncertainty){
      
      gg2 <- gg2 + geom_line(data = final_pred_sum[location_id == loc_id], aes(y = resid_mean, color = antigen))+
        geom_ribbon(data = final_pred_sum[location_id == loc_id], aes(ymin = resid_lower, ymax = resid_upper, fill = antigen), alpha = 0.15, show.legend = F)
      
      
      gg4 <- gg4 + geom_line(data = final_pred_sum[location_id == loc_id], aes(y = cum_ratio_2_mean, color = antigen, linetype = "Stage 2"))+
        geom_ribbon(data = final_pred_sum[location_id == loc_id], aes(ymin = cum_ratio_2_lower, ymax = cum_ratio_2_upper, fill = antigen), alpha = 0.15, show.legend = F)
      
      
      gg6 <- gg6 + geom_line(data = final_pred_sum[location_id == loc_id], aes(y = month_ratio_2_mean, color = antigen, linetype = "Stage 2"))+
        geom_ribbon(data = final_pred_sum[location_id == loc_id], aes(ymin = month_ratio_2_lower, ymax = month_ratio_2_upper, fill = antigen), alpha = 0.15, show.legend = F)
      
      
    }else{
      
      gg4 <- gg4 + geom_line(aes(y = ratio, linetype = "Stage 2", color = antigen))
      
      gg6 <- gg6 + geom_line(aes(y = month_ratio, color = antigen, linetype = "Stage 2"))
      
      if(loc_id %in% resid_dt$location_id){
        gg2 <- gg2 + geom_line(aes(y = pred, color = antigen, linetype = "Country"))
      }
    }
    
    p_all <- plot_grid(gg1, gg2, gg3, gg4, gg5, gg6, ncol = 2, rel_widths = c(1,1.3))
    
    title <- ggdraw() + draw_label(paste0(loc_name, ", ", r_name), fontface='bold')
    
    p_all <- plot_grid(title, p_all, ncol=1, rel_heights=c(0.05, 1)) # rel_heights values control title margins
...
print(best_knots1)*max_mob
print(best_knots2)*9+3
